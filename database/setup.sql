-- Production database initialization and complete ruleset seeding script
-- Target engine: InnoDB with UTF8MB4 encoding to guarantee transaction safety and complete Unicode alignment

CREATE DATABASE IF NOT EXISTS `privacyknowledgebase`
    DEFAULT CHARACTER SET utf8mb4
    COLLATE utf8mb4_unicode_ci;

USE `privacyknowledgebase`;

-- --------------------------------------------------------------------
-- Users Table (System Administrators & Security Managers)
-- --------------------------------------------------------------------
DROP TABLE IF EXISTS `personal_informations`;
CREATE TABLE `personal_informations` (
    `id` INT(11) NOT NULL AUTO_INCREMENT,
    `username` VARCHAR(50) NOT NULL UNIQUE,
    `role` ENUM('admin', 'security manager') NOT NULL,
    `password` VARCHAR(255) NOT NULL,
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- Seed default operator accounts mapping baseline staff authorization constraints
-- Note to self: Retaining plaintext strings temporarily to support instant dev offline verification flows.
-- Must refactor to output verification buffers using password_hash() securely prior to production server deployment.
INSERT INTO `personal_informations` (`username`, `role`, `password`) VALUES
('admin', 'admin', 'adminpass123'),
('security_manager', 'security manager', 'secpass123');

-- --------------------------------------------------------------------
-- Patterns Table (Core Privacy Knowledge Base Ruleset Mapping)
-- --------------------------------------------------------------------
...
CREATE TABLE `patterns` (
    ...
    `iso_phase` VARCHAR(255) DEFAULT NULL,
    `gdpr_article` VARCHAR(255) DEFAULT NULL,
    `privacy_principles` VARCHAR(255) DEFAULT NULL,
    `owasp_categories` VARCHAR(255) DEFAULT NULL,
    `cwe_categories` TEXT DEFAULT NULL,
    `examples` TEXT DEFAULT NULL,
    `collocazione_mvc` VARCHAR(100) DEFAULT NULL,
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    -- Applying a Full-Text index over search query targets to enforce sub-millisecond API response latency
    FULLTEXT KEY `idx_search` (`name`, `description`, `strategies`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- Injecting the absolute mapping of all 25 core design patterns matching extracted legacy specifications perfectly
INSERT INTO `patterns` (
    `id`, `name`, `strategies`, `description`, `context`, `iso_phase`,
    `gdpr_article`, `privacy_principles`, `owasp_categories`, `cwe_categories`, `examples`, `collocazione_mvc`
) VALUES 
(
    1,
    'Protection against Tracking',
    'Minimize',
    'This Pattern avoids tracking website visitors by cookies. It does this by deleting them at regular intervals or disabling cookies completely.',
    ...
    '1.-->Privacy as the default setting, 2.-->Privacy Embedded into Design, 3.-->Visibility and Transparency, 4.-->Proactive not Reactive',
    'A05-->Security Misconfiguration, A07-->Identification and Authentication Failures',
    'CWE-306-->Missing Authentication for Critical Function, CWE-798-->Use of Hard-coded Credentials, CWE-287-->Improper Authentication',
    'Example 1-->Alice wants to buy shoes and she wants to shop online. She heads to an online shop and searches for shoes but cant decide which ones she wants, so she buys neither of them. The next day she finds a couple of emails in her inbox, giving her suggestions for other shoes and alerting her that the viewed shoes are now on sale.',
...
(
    ...
    'Minimize',
    'Delete potentially sensitive metadata that is not directly visible to the end user.',
    'When a service requires a user to import data from external sources (e.g., images, tweets, documents), different types of metadata may be transmitted. Users may not be aware of metadata since it may be automatically generated or not directly visible. Services may be unintentionally responsible for exposing private metadata or going against users expectations.',
    '7.4-->Producing design solutions',
    'Article 25-->Data protection by design and by default, Article 5-->Principles relating to processing of personal data',
    '1.-->Respect for User Privacy, 2.-->Visibility and Transparency, 3.-->Privacy Embedded into Design',
    'A04-->Insecure Design, A05-->Security Misconfiguration',
    'CWE-269-->Improper Privilege Management, CWE-434-->Unrestricted Upload of File with Dangerous Type',
    'Example 1-->Twitter.com removes EXIF data from images uploaded to their image sharing service. Previously there have been many breaches of personal location by using EXIF data shared by image sharing services, Example 2-->Flickr.com is a system features based on metadata, the metadata sharing is an important part of the community of users. Flickr.com allows users to hide their EXIF data from public display, and also provides an interface for users to easily see whether they are sharing location as part of uploading their images.',
    'Controller, Model'
),
(
    3,
    ...
    '7.4-->Producing design solutions',
    'Article 32-->Security of processing',
    '1.-->Privacy Embedded into Design, 2.-->Visibility and Transparency',
    'A04-->Insecure Design, A05-->Security Misconfiguration, A09-->Security Logging and Monitoring Failures, A07-->Identification and Authentication Failures',
    'CWE-269-->Improper Privilege Management, CWE-434-->Unrestricted Upload of File with Dangerous Type, CWE-306-->Missing Authentication for Critical Function, CWE-798-->Use of Hard-coded Credentials, CWE-287-->Improper Authentication',
    'Example 1-->An electric utility operates a smart grid network with smart meters that provide measurements of the instantaneous power consumption of each user. The utility employs that information to both adapt the power distribution in a dynamic fashion, according to user demand at each moment, and bill the each client periodically, according to his aggregated consumption over the billing period. However, this information can also be exploited to infer sensitive user information (e.g. at what time he or she leaves and comes back to home, etc.)',
    'Model'
),
(
    4,
    ...
    '1.-->Visibility and Transparency, 2.-->Proactive not Reactive',
    'A05-->Security Misconfiguration, A08-->Software and Data Integrity Failures, A09-->Security Logging and Monitoring Failures',
    'CWE-502-->Deserialization of Untrusted Data',
    'Example 1-->Assume a [company] stores all employees data [through a controllers service]. There is a contractual agreement between [them] that each data leakage is reported within one hour. Now Bob, an employee of [the controller] and not authorized to read [the companys] data, succeeds in circumventing [the] access control mechanisms and reads [personal] data. This represents a data breach of which [the company] has to be notified within an hour.',
    'Controller, Model'
),
(
    5,
    'Unusual Activities',
    'Inform',
    'Prevent suspicious access to user data through alerts and perform authentication through multiple factors in case of potential account compromise.',
    'Services (or products), especially on the Internet, tend to use username and password-based authentication. This security mechanism is very convenient for users, as it is widespread and simple compared to more secure alternatives. However, it is subject to common flaws. Passwords become less secure as they remain unchanged, are often vulnerable to brute force attacks, espionage and phishing, and cannot be proven to be held exclusively by the user. This complicates the certainty of authentication, and thus the authenticity of any decision made by the user, including consent. Controllers may derive additional factors, such as device- or access-specific information. If location is provided, for example, it might suggest unlikely account activity.',
    '7.3-->Specify the requirements, 7.4-->Producing design solutions',
    'Article 32-->Security of processing',
    '1.-->Proactive not Reactive, 2.-->Respect for User Privacy, 3.-->End-to-End Security',
    'A01-->Broken Access Control, A07-->Identification and Authentication Failures, A09:Security Logging and Monitoring Failures',
    'CWE-22-->Improper Limitation of a Pathname to a Restricted Directory (Path Traversal), CWE-862-->Missing Authorization, CWE-863-->Incorrect Authorization, CWE-276-->Incorrect Default Permissions, CWE-306-->Missing Authentication for Critical Function, CWE-798-->Use of Hard-coded Credentials, CWE-287-->Improper Authentication',
    'Example 1-->Gmail displays information about other sessions (if any) in the footer, linking to a page named (Activity on this account) which lists other sessions and recent activities to the Gmail account. The user has the option to sign out other sessions. In case of annoying false positives, the user may choose to disable the alert for unusual activity. The disable takes about a week, (to make sure the bad guys arent the ones who turned off your alerts.), Example 2-->When Facebook detects an unusual sign-in, it shows social authentication that displays a few pictures of the users friends and asks the user to name the person in those photos, Example 3-->The Security tab of the (Settings) of the Dropbox website displays all web browser sessions logged in to the account, and enables the user to log out one or more of them. The name of the browser, operating system, and the IP address and corresponding country are displayed to help the user make a choice. It also displays all devices that are linked to the account, and allows the user to unlink one or more of them.',
    'Controller, Model'
),
(
    6,
...
(
    ...
    'Onion Routing',
    'Hide',
    'This model ensures nonlinkability between senders and receivers by encapsulating data in different layers of encryption, limiting the knowledge of each node along the delivery path.',
    'A system in which data is routed between different nodes.',
    '7.4-->Producing design solutions',
    'Article 25-->Data protection by design and by default, Article 32-->Security of processing',
    '1.-->Privacy Embedded into Design, 2.-->Proactive not Reactive, 3.-->End-to-End Security',
    'A02-->Cryptographic Failures, A04-->Insecure Design, A05-->Security Misconfiguration, A08-->Software and Data Integrity Failures, A07-->Identification and Authentication Failures',
    'CWE-269-->Improper Privilege Management, CWE-434-->Unrestricted Upload of File with Dangerous Type, CWE-306-->Missing Authentication for Critical Function, CWE-798-->Use of Hard-coded Credentials, CWE-287-->Improper Authentication, CWE-502-->Deserialization of Untrusted Data',
    'Example 1-->Alice is a whistle-blower and tries to forward data to Bob who works at the press. She sends the corresponding documents as an e-mail-attachment. Eve monitors the traffic and can see who sent this mail to whom. The next day, police raids Alices apartment and sends her to jail. Bobs mail account gets seized, Example 2-->The TOR-browser, a web-browser specifically designed to ensure anonymity makes heavy use of onion routing.',
    'Controller, Model'
),
(
    8,
    ...
    'Hide, Control',
    'Use encryption in such a way that the service provider cannot decrypt the users information because the user manages the keys.',
    ...
    '1.-->Full Functionality, 2.-->End-to-End Security',
    'A02-->Cryptographic Failures, A04-->Insecure Design',
    'CWE-269-->Improper Privilege Management, CWE-434-->Unrestricted Upload of File with Dangerous Type',
    'Example 1-->Least Authority: secure off-site backup system with client side encryption, Example 2-->LastPass: encrypted credentials and personal information database with user managed encryption keys.',
    'Controller, Model'
),
(
    9,
    ...
    '7.4-->Producing design solutions',
    'Article 32-->Security of processing',
    '1.-->Proactive not Reactive, 2.-->Privacy as the default setting, 3.-->Respect for User Privacy, 4.-->Privacy Embedded into Design',
    'A01-->Broken Access Control, A03-->Injection, A05-->Security Misconfiguration, A07-->Identification and Authentication Failures, A10-->Server-Side Request Forgery (SSRF)',
    'CWE-22-->Improper Limitation of a Pathname to a Restricted Directory (Path Traversal), CWE-862-->Missing Authorization, CWE-863-->Incorrect Authorization, CWE-276-->Incorrect Default Permissions, CWE-20-->Improper Input Validation, CWE-79-->Improper Neutralization of Input During Web Page Generation (Cross-site Scripting), CWE-89-->Improper Neutralization of Special Elements used in an SQL Command (SQL Injection), CWE-78-->Improper Neutralization of Special Elements used in an OS Command (OS Command Injection), CWE-20-->Improper Input Validation, CWE-77-->Improper Neutralization of Special Elements used in a Command (Command Injection), CWE-94-->Improper Control of Generation of Code (Code Injection), CWE-306-->Missing Authentication for Critical Function, CWE-798-->Use of Hard-coded Credentials, CWE-287-->Improper Authentication, CWE-918-->Server-Side Request Forgery (SSRF)',
    'Example 1-->Alice wants to search for an abortion clinic on Google, but she does not want to reveal her intentions of abort to an adversary that may be eavesdropping this search (e.g., ISP provider, system administrator of her workplace, etc).',
    'Controller, Model'
),
(
    10,
...
(
    ...
    'A05-->Security Misconfiguration, A07-->Identification and Authentication Failures, A08-->Software and Data Integrity Failures, A09-->Security Logging and Monitoring Failures',
    'CWE-306-->Missing Authentication for Critical Function, CWE-798-->Use of Hard-coded Credentials, CWE-287-->Improper Authentication, CWE-502-->Deserialization of Untrusted Data, CWE-502-->Deserialization of Untrusted Data',
    'Example 1-->A service provider subcontracts services, but requires that the data to be deleted after a certain time and that the service provider requires to be notified if there is further subcontracting.',
    'Controller, Model'
),
(
    12,
...
(
    ...
    '7.4-->Producing design solutions',
    'Article 25-->Data protection by design and by default',
    '1.-->Privacy Embedded into Design, 2.-->Respect for User Privacy, 3.-->Privacy as the default setting',
    'A08-->Software and Data Integrity Failures, A09-->Security Logging and Monitoring Failures',
    'CWE-502-->Deserialization of Untrusted Data',
    'Example 1-->In the health sector, patients want to keep control over their health data but also to grant specific access to some health professionals.',
    'Model'
),
(
    14,
    ...
    '7.4-->Producing design solutions',
    'Article 32-->Security of processing',
    '1.-->Proactive not Reactive, 2.-->Privacy Embedded into Design',
...
(
    ...
    'CWE-22-->Improper Limitation of a Pathname to a Restricted Directory (Path Traversal), CWE-862-->Missing Authorization, CWE-863-->Incorrect Authorization, CWE-276-->Incorrect Default Permissions',
    'Example 1-->Yahoo! Fire Eagle allows user to provide location information to applications using eight different (levels) of granularity in their hierarchy: No information, As precise as possible, Postal code, Neighborhood, Town, Region, State and Country. Fire Eagle specifically requires that recipient applications be written to handle data at any of the levels, and allows updating the users location at any level of granularity, Example 2-->Twitter allows users to tag a tweet with either exact coordinates, a Twitter (place) (a town, neighborhood or venue) or both, Example 3-->One of the fore-runners to the W3C Geolocation API, Firefoxs experimental Geode feature allowed JavaScript access to the current location at four different levels of granularity.',
    'Controller, Model'
),
(
    17,
    ...
    '1.-->Privacy Embedded into Design, 2.-->Respect for User Privacy',
    'A04-->Insecure Design, A07-->Identification and Authentication Failures',
    'CWE-269-->Improper Privilege Management, CWE-434-->Unrestricted Upload of File with Dangerous Type, CWE-306-->Missing Authentication for Critical Function, CWE-798-->Use of Hard-coded Credentials, CWE-287-->Improper Authentication',
    'Example 1-->On Facebook, before publishing a post, you can set one of three levels of privacy for the post which can be (Everyone), (Friends Only), (Private). In the first case the post can be seen by any Facebook user, in the second case the post can only be seen by the friends of the post user, finally, in the last case the post is not visible to anyone except the owner.',
    'Controller, View'
),
(
    18,
...
(
    ...
    '7.2-->Understanding and specifying the context of use, 7.3-->Specify the requirements',
    'Article 13-->Information to be provided where personal data are collected from the data subject',
    '1.-->Respect for User Privacy, 2.-->Privacy as the default setting, 3.-->Privacy Embedded into Design',
    'A05-->Security Misconfiguration, A07-->Identification and Authentication Failures, A08-->Software and Data Integrity Failures',
    'CWE-306-->Missing Authentication for Critical Function, CWE-798-->Use of Hard-coded Credentials, CWE-287-->Improper Authentication, CWE-502-->Deserialization of Untrusted Data',
    'Example 1-->We hire a healthcare center, data controller, an interested user and a subcontractor, provider of sensor networks for monitoring user health. This subcontractor requires additional specific, informed, explicit and freely given consent from you.',
    'Controller, Model'
),
(
    21,
    ...
    'Users do not inherently trust controllers who provide services (or products), as they do not have assurances as to what the controllers or their processors true intentions are. Controllers and processors typically aim to make profit, but this might be at the expense of users if those users do not consider their privacy needs. The controller might have reasonable defaults or levels of control, but users also need to feel reassured that their choices are being honored. This is especially true of what they do or do not provide Lawful Consent for.',
    '7.2-->Understanding and specifying the context of use',
    'Article 6-->Lawfulness of processing, Article 7-->Conditions for consent',
    '1.-->Proactive not Reactive, 2.-->Privacy Embedded into Design, 3.-->Respect for User Privacy, 4.-->Visibility and Transparency',
    'A01-->Broken Access Control, A02-->Cryptographic Failures, A03-->Injection, A07-->Identification and Authentication Failures',
    'CWE-22-->Improper Limitation of a Pathname to a Restricted Directory (Path Traversal), CWE-862-->Missing Authorization, CWE-863-->Incorrect Authorization, CWE-276-->Incorrect Default Permissions, CWE-20-->Improper Input Validation, CWE-79-->Improper Neutralization of Input During Web Page Generation (Cross-site Scripting), CWE-89-->Improper Neutralization of Special Elements used in an SQL Command (SQL Injection), CWE-78-->Improper Neutralization of Special Elements used in an OS Command (OS Command Injection), CWE-20-->Improper Input Validation, CWE-77-->Improper Neutralization of Special Elements used in a Command (Command Injection), CWE-94-->Improper Control of Generation of Code (Code Injection), CWE-306-->Missing Authentication for Critical Function, CWE-798-->Use of Hard-coded Credentials, CWE-287-->Improper Authentication',
    NULL,
    'Controller, Model'
),
(
    ...
    '7.4-->Producing design solutions',
    'Article 32-->Security of processing',
    '1.-->Privacy as the default setting, 2.-->Respect for User Privacy, 3.-->Proactive not Reactive',
    'A01-->Broken Access Control, A02-->Cryptographic Failures, A08-->Software and Data Integrity Failures, A09-->Security Logging and Monitoring Failures',
    'CWE-22-->Improper Limitation of a Pathname to a Restricted Directory (Path Traversal), CWE-862-->Missing Authorization, CWE-863-->Incorrect Authorization, CWE-276-->Incorrect Default Permissions, CWE-502-->Deserialization of Untrusted Data',
    'Example 1-->Unmonitored communications systems to ensure free speech. Pseudonymous messages do not allow us to trace the sender of the message.',
    'Model'
),
(
    23,
    ...
    '7.4-->Producing design solutions',
    'Article 25-->Data protection by design and by default, Article 32-->Security of processing',
    '1.-->Privacy as the default setting, 2.-->Respect for User Privacy',
    'A05-->Security Misconfiguration, A07-->Identification and Authentication Failures',
    'CWE-306-->Missing Authentication for Critical Function, CWE-798-->Use of Hard-coded Credentials, CWE-287-->Improper Authentication',
    'Example 1-->Aims to provide some privacy, but it is important to be aware of the possible vulnerabilities in using a pseudonym , which can be countered by implementing additional security measures if necessary.',
    'Controller, Model'
),
(
    24,
    ...
    '7.4-->Producing design solutions',
    'Article 32-->Security of processing',
    '1.-->End-to-End Security, 2.-->Privacy as the default setting',
    'A06-->Vulnerable and Outdated Components, A08-->Software and Data Integrity Failures',
    'CWE-502-->Deserialization of Untrusted Data',
    'Example 1-->An electric utility operates a smart grid network with smart meters that provide measurements of the instantaneous power consumption of each user. The utility employs that information to adapt the power distribution in a dynamic fashion, according to the user demand at each moment.',
    'Controller, Model'
),
(
    25,
    ...
    'Article 25-->Data protection by design and by default',
    '1.-->Privacy Embedded into Design, 2.-->Visibility and Transparency',
    'A02-->Cryptographic Failures, A03-->Injection, A05-->Security Misconfiguration',
    'CWE-20-->Improper Input Validation, CWE-79-->Improper Neutralization of Input During Web Page Generation (Cross-site Scripting), CWE-89-->Improper Neutralization of Special Elements used in an SQL Command (SQL Injection), CWE-78-->Improper Neutralization of Special Elements used in an OS Command (OS Command Injection), CWE-20-->Improper Input Validation, CWE-77-->Improper Neutralization of Special Elements used in a Command (Command Injection), CWE-94-->Improper Control of Generation of Code (Code Injection)',
    'Example 1-->Every system that involves the use of a password to protect personal data should implement a password generation tool. This system, in addition to generating passwords, must educate users to create secure passwords.',
    'Controller, View'
);

-- --------------------------------------------------------------------
-- Feedback Table (Security Monitoring Interface Mapping)
-- --------------------------------------------------------------------
...
-- Seed an internal setup verification check record directly accessible via Security Manager notifications desk
INSERT INTO `feedback` (`feedback_type`, `feedback_content`) VALUES
('System Verification Log', 'Initial database ruleset compilation complete. Application controllers map DDL structural integrity correctly.');